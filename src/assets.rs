include!(concat!(env!("OUT_DIR"), "/templeos_assets.rs"));

use std::{collections::HashMap, sync::OnceLock};

// CP437 → Unicode mapping used by TempleOS' 8×8 `sys_font_std`.
//
// Notes:
// - This table is intentionally single-byte (not UTF-8). Many vendored TempleOS sources contain
//   raw CP437 bytes (e.g. box drawing in `::/Kernel/KMain.HC`), so treating them as UTF-8 can
//   corrupt char literals and string contents.
// - We mostly care about the ability to roundtrip Unicode ↔ CP437 for rendering.
const CP437_TO_UNICODE: [u16; 256] = [
...
fn cp437_reverse_map() -> &'static HashMap<char, u8> {
    static MAP: OnceLock<HashMap<char, u8>> = OnceLock::new();
    MAP.get_or_init(|| {
        let mut out: HashMap<char, u8> = HashMap::with_capacity(256);
        for (i, &cp) in CP437_TO_UNICODE.iter().enumerate() {
            let Some(ch) = char::from_u32(cp as u32) else {
                continue;
            };
            out.insert(ch, i as u8);
        }
        out
    })
}

pub fn decode_cp437_byte(b: u8) -> char {
    char::from_u32(CP437_TO_UNICODE[b as usize] as u32).unwrap_or('?')
}

pub fn decode_cp437_bytes(bytes: &[u8]) -> String {
    let mut out = String::with_capacity(bytes.len());
    for &b in bytes {
        out.push(decode_cp437_byte(b));
    }
    out
}
...
pub fn encode_cp437(ch: char) -> u8 {
    ...
    cp437_reverse_map().get(&ch).copied().unwrap_or(b'?')
}

pub fn sys_font_std_glyph_row_bits(glyph: u8, row: u8) -> u8 {
    let row = row & 7;
    let bits = TEMPLEOS_SYS_FONT_STD_U64[glyph as usize] >> ((row as u32) * 8);
    (bits & 0xFF) as u8
}
